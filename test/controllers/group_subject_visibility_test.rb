...
class GroupSubjectVisibilityTest < ActionDispatch::IntegrationTest
  ...
  test "non-member cannot edit a group subject via direct URL" do
    ...
    assert_equal "Subject not found", flash[:alert]
  end
  ...
  test "non-member cannot update a group subject via direct URL" do
    ...
    assert_redirected_to student_subjects_path(@student_not_in_group)
    assert_equal "Subject not found", flash[:alert]
    assert_equal "Science (Family)", @group_subject.reload.name, "Subject should not be modified"
  end
  ...
  test "Student#all_subjects includes subjects from all joined groups" do
    ...
    subjects = @student_in_group.all_subjects
    subject_names = subjects.pluck(:name)

    assert_includes subject_names, "Science (Family)", "Should include family group subject"
  ...
  test "student only sees subjects from groups they belong to" do
    ...
    assert_includes subjects, @group_subject, "Should include family_group subject (Jordan is a member)"
    assert_not_includes subjects, joint_subject, "Should NOT include joint_group subject (Jordan is not a member)"
  ensure
